...
const filterStringsWithVowels = (arr) => {
  // Solution code here...
  var re = /[aeiou]/;
  const result = arr.filter(word => re.test(word));
  return result;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  ...
  if(forbiddenValues.length === 0 || arr.length === 0){
    return arr;
  }
  const result = arr.filter(word => !forbiddenValues.includes(word));
  return result;
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  // Solution code here...
  return arr.filter(obj => minBaseStat < obj.baseStat);
};
...
const getStatName = (arr, minBaseStat) => {
  // Solution code here...
  return arr.filter(obj => minBaseStat < obj.baseStat).map(obj => obj.stat.name)
};
